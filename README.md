...
Бот написан с помощью библиотеки aiogram. Включает весь неоходимый функционал:
...
ID администратора и ряд команд сможет совершать только он. После каждого повтрного запуска ID обновляется, поэтому необходимо проходить проверку повторно. 
...
1. Просмотру поступивих заявок
2. Изменению статуса заявок
3. Удалению заявок
Более подробно о БД в следующем пункте.  


P.S. Если Вы захотите прописать валидацию и к другим хендлерам, то необходимо в хендлере прописать условие `if message.from_user.id == ID`, сделать импорт переменной **ID** и перенести хендлер в файл *admin.py*.
